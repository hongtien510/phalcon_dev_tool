...
class Scaffold extends Component
{
     ...
     * @return string
     */
    private function _captureFilterInput($var, $fields, $useGetSetters, $identityField, $type)
    {
        $code = '';
        foreach ($fields as $field => $dataType) {
            ...
            if (strpos($dataType, 'int') !== false) {
				...
				if($dataType == 0){
					$fieldCode = '$this->request->getPost("'.$field.'", "int", NULL, true)';
				}elseif($dataType == 4){
					if($type == 'update'){
						if(in_array($field, array('modified', 'modified_at'))){
							$fieldCode = "date('Y-m-d H:i:s')";
						}
						else{
							$fieldCode = "";
						}
    ...
    private function _makeField($attribute, $dataType, $relationField, $selectDefinition)
    {
		if($attribute != 'modified' && $attribute != 'created'){
			$code = "\t" . '<div class="form-group">' . PHP_EOL .
					"\t\t" . '<label class="col-md-3 control-label" for="' . $attribute . '">' . $this->_getPossibleLabel($attribute) . '</label>' . PHP_EOL .
					"\t\t" . '<div class="col-md-9">';
			...
			if (isset($relationField[$attribute])) {
				...
				switch ($dataType) {
					...
					case Column::TYPE_CHAR:
						$code .= PHP_EOL . "\t\t\t\t" . '<?php echo $this->tag->textField(array("' . $attribute . '", "class" => "form-control", "placeholder" => "'.mb_convert_case($attribute, MB_CASE_TITLE, "UTF-8").'")) ?>';
						break;
					case Column::TYPE_DECIMAL:
					case Column::TYPE_INTEGER:
						$code .= PHP_EOL . "\t\t\t" . '<?php echo $this->tag->textField(array("' . $attribute . '", "type" => "number", "class" => "form-control", "placeholder" => "'.mb_convert_case($attribute, MB_CASE_TITLE, "UTF-8").'")) ?>';
						break;
					case Column::TYPE_DATE:
						$code .= PHP_EOL . "\t\t\t\t" . '<?php echo $this->tag->textField(array("' . $attribute . '", "type" => "date", "class" => "form-control", "placeholder" => "'.mb_convert_case($attribute, MB_CASE_TITLE, "UTF-8").'")) ?>';
						break;
					case Column::TYPE_TEXT:
						$code .= PHP_EOL . "\t\t\t\t" . '<?php echo $this->tag->textArea(array("' . $attribute . '", "cols" => 30, "rows" => 4, "class" => "form-control", "placeholder" => "'.mb_convert_case($attribute, MB_CASE_TITLE, "UTF-8").'")) ?>';
						break;
					default:
						$code .= PHP_EOL . "\t\t\t" . '<?php echo $this->tag->textField(array("' . $attribute . '", "class" => "form-control", "placeholder" => "'.mb_convert_case($attribute, MB_CASE_TITLE, "UTF-8").'")) ?>';
						break;
    ...
    private function _makeFields($action)
    {
        ...
        foreach ($this->options->dataTypes as $attribute => $dataType) {
            if (($action == 'new' || $action == 'edit' || $action == 'view') && $attribute == $identityField) {
                continue;
            }
			if($action == 'view'){
				$code .= $this->_makeFieldView($attribute, $dataType, $relationField, $selectDefinition);
			}
			else{
    ...
    private function _makeController()
    {
        ...
        $code = str_replace('$assignInputFromRequestCreate$', $this->_captureFilterInput($this->options->singular, $this->options->dataTypes, $this->options->genSettersGetters, $this->options->identityField, 'create'), $code);
        $code = str_replace('$assignInputFromRequestUpdate$', $this->_captureFilterInput($this->options->singular, $this->options->dataTypes, $this->options->genSettersGetters, $this->options->identityField, 'update'), $code);
	...
	private function _makeViewView()
    {
        $dirPath = $this->options->viewsDir . $this->options->fileName;
        if (is_dir($dirPath) == false) {
            mkdir($dirPath);
        ...
        $viewPath = $dirPath . DIRECTORY_SEPARATOR . 'view.phtml';
        if (file_exists($viewPath) && !$this->options->contains('force')) {
            return;
        }

        $templatePath = $this->options->templatePath . '/scaffold/no-forms/views/view.phtml';
        if (!file_exists($templatePath)) {
            throw new BuilderException(sprintf('Template "%s" does not exist', $templatePath));
        }
        ...
        foreach ($this->options->dataTypes as $fieldName => $dataType) {
            ...
            if (!isset($this->options->allReferences[$fieldName])) {
                if ($this->options->genSettersGetters) {
					...
					if($fieldName == 'image'){
						$rowCode = substr($rowCode,0, -5);//REMOVE echo
						$rowCode .='if(file_exists($'.$this->options->singular.'->image)){ ?>
				<a class="fancybox" href="<?php echo $this->url->getBaseUri() . $'.$this->options->singular.'->image ?>">
					<img height="50" src="<?php echo $this->url->getBaseUri() . $'.$this->options->singular.'->image ?>"/>
				</a>
				<?php } else{ ?>
					<img height="50" src="<?php echo $this->url->getBaseUri() ?>images/no-image.jpg" />
				<?php }
			';
					}
					else{
